		...
		while self.running.is_set():
			while not self.stopped:
				...
				if dst_points is not None:
					...
					self.dst_queue.put(self.dst_img)
	def stop(self):
		# Release the video capture and destroy the cv2 window
		print('ending face swap')
		self.stopped = True
		print('closing windows')
		cv2.destroyAllWindows()
		print('closing syphon server')
		server2.should_close = True
		#stop the syphon server
		#server2.should_close = True

def timer_function(duration, video_handler):
	timer = threading.Timer(duration, video_handler.stop)
	timer.start()
	video_handler.start()
...
if __name__ == '__main__':
	...
	print('sarting video handler')
	video_handler = VideoHandler(video_path=0, img_path='interactive/data/dream.jpg', args=args)
	timer_function(5, video_handler)
